...
# Embedded file name: .\src\test\rm_ctrl.py
# Compiled at: 2022-02-11 15:12:26
# Size of source mod 2**32: 291759 bytes
import tools, time, rm_module, rm_define, math, threading, functools, rm_log, rm_builtins, re, duml_cmdset
    ...
    def wrapper(*args, **kw):
        ...
        if task_finish == True:
            ...
            if 'err_code' in task_dict['kw'].keys():
                ...
                if ctrl.event_client.script_state.get_block_running_fail_reason_code() != 0:
        ...
        return (
         has_event, task_finish, robot_task, result)
    ...
    def set_action_state(self, action):
        ...
        for act in self.action_state.keys():
            if self.action_state[act]:
                if act in self.mutex_action[action]:
...
class RobotCtrlTool(RobotCtrlBases):
    ...
    def sleep(self, sleep_time):
        CHECK_VALUE_RANGE_AND_TYPE(sleep_time, 0, 3600, int, float)
        t1 = time.time()
...
class ChassisCtrl(RobotCtrlTool):
    ...
    def __init__(self, event_client):
        ...
        self.event_client = event_client
        self.attr_dict = {'mode':rm_define.chassis_sdk_free_mode, 
    ...
    def rotate(self, direction):
        ...
        CHECK_VALUE_IN_ENUM_LIST(direction, rm_define.clockwise=rm_define.clockwise, 
    ...
    def rotate_with_degree(self, direction, degree):
        ...
        if self.attr_dict['mode'] != rm_define.chassis_sdk_follow_mode:
            ...
            if direction == rm_define.anticlockwise:
                ...
                if direction == rm_define.clockwise:
                    degree = -degree
                else:
                    pass
            self.set_action_state('rotate')
    ...
    def get_wheel_speed(self, wheel_num=None):
        ...
        if wheel_num == rm_define.chassis_wheel_2:
            return self.status_dict['cur_speed_wheel2']
        if wheel_num == rm_define.chassis_wheel_3:
            return self.status_dict['cur_speed_wheel3']
        if wheel_num == rm_define.chassis_wheel_4:
    ...
    def get_speed(self, direction):
        ...
        if direction == rm_define.chassis_translation:
            return self.status_dict['cur_speed_by']
        if direction == rm_define.chassis_rotate:
    ...
    def get_position_based_power_on(self, direction=None):
        ...
        CHECK_VALUE_IN_ENUM_LIST(direction, rm_define.chassis_forward=rm_define.chassis_forward, 
        ...
        if direction == rm_define.chassis_rotate:
            ...
            if self.status_dict['cur_attitude_yaw'] - self.status_dict['init_attitude_yaw'] > 180:
    ...
    def get_attitude(self, attitude=None):
        ...
        CHECK_VALUE_IN_ENUM_LIST(attitude, rm_define.chassis_pitch=rm_define.chassis_pitch, 
        ...
        if attitude == rm_define.chassis_roll:
            return self.status_dict['cur_attitude_roll']
        if attitude == rm_define.chassis_yaw:
    ...
    def attitute_status_process(self, event_client, msg):
        ...
        self.attitude_status_dict['static_flag'] = status >> 0 & 1
        self.attitude_status_dict['uphill_flag'] = status >> 1 & 1
        self.attitude_status_dict['downhill_flag'] = status >> 2 & 1
        self.attitude_status_dict['on_slope_flag'] = status >> 3 & 1
        self.attitude_status_dict['pick_up_flag'] = status >> 4 & 1
        self.attitude_status_dict['slip_flag'] = status >> 5 & 1
        self.attitude_status_dict['impact_x_flag'] = status >> 6 & 1
        self.attitude_status_dict['impact_y_flag'] = status >> 7 & 1
        self.attitude_status_dict['impact_z_flag'] = status >> 8 & 1
        self.attitude_status_dict['roll_over'] = status >> 9 & 1
        self.attitude_status_dict['hill_static'] = status >> 10 & 1
        if self.attitude_status_dict['impact_x_flag'] or self.attitude_status_dict['impact_y_flag']:
    ...
    def update_attitude_status(self, status_dict):
        ...
        if self.sdk_info_push_callback:
            if self.sdk_position_info_push_attr['enable_flag']:
                if self.update_attitude_status_count % int(self.sdk_position_info_push_attr['freq_count']) == 0:
                    info['position'] = self.get_position()[0:2]
            if self.sdk_attitude_info_push_attr['enable_flag']:
                if self.update_attitude_status_count % int(self.sdk_attitude_info_push_attr['freq_count']) == 0:
                    info['attitude'] = self.get_attitude()
            if self.sdk_status_info_push_attr['enable_flag']:
                if self.update_attitude_status_count % int(self.sdk_status_info_push_attr['freq_count']) == 0:
            ...
            if self.attr_dict['mode'] == rm_define.chassis_sdk_free_mode:
                if self.status_dict['stick_flag'] == True:
                    if self.attr_dict['stick_overlay_need_update_mode'] == True:
                        ...
                        if not self.check_action_state('rotate'):
                            self.attr_dict['stick_overlay_need_update_mode'] = self.check_action_state('move') or self.check_action_state('move_rotate') or False
                    ...
                    if self.attr_dict['stick_overlay_need_update_mode'] == False:
                        ...
                        if not self.check_action_state('rotate'):
                            if not self.check_action_state('move'):
                                if not self.check_action_state('move_rotate'):
                                    self.attr_dict['stick_overlay_need_update_mode'] = True
                                    self._set_chassis_stop_depend_mode()
    ...
    def _move_ctrl(self, func_start, t=rm_define.time_forever):
        ...
        if self.attr_dict['m_dire'] == rm_define.chassis_front:
            ...
            pass
        if self.attr_dict['m_dire'] == rm_define.chassis_back:
            speed_x = -self.attr_dict['m_speed']
        else:
            if self.attr_dict['m_dire'] == rm_define.chassis_right:
                speed_y = self.attr_dict['m_speed']
            else:
                if self.attr_dict['m_dire'] == rm_define.chassis_left:
                    speed_y = -self.attr_dict['m_speed']
                else:
                    if self.attr_dict['m_dire'] == rm_define.chassis_left_front:
                        speed_x = self.attr_dict['m_speed'] / 1.414
                        speed_y = -self.attr_dict['m_speed'] / 1.414
                    else:
                        if self.attr_dict['m_dire'] == rm_define.chassis_left_back:
                            speed_x = -self.attr_dict['m_speed'] / 1.414
                            speed_y = -self.attr_dict['m_speed'] / 1.414
                        else:
                            if self.attr_dict['m_dire'] == rm_define.chassis_right_front:
                                speed_x = self.attr_dict['m_speed'] / 1.414
                                speed_y = self.attr_dict['m_speed'] / 1.414
                            else:
                                if self.attr_dict['m_dire'] == rm_define.chassis_right_back:
                                    speed_x = -self.attr_dict['m_speed'] / 1.414
                                    speed_y = self.attr_dict['m_speed'] / 1.414
                                else:
                                    if self.attr_dict['m_dire'] == rm_define.chassis_customize_direction:
    ...
    def _rotate_ctrl(self, func_start, t=rm_define.time_forever, speed=None):
        ...
        if self.attr_dict['r_dire'] == rm_define.clockwise:
            ...
            if self.attr_dict['r_dire'] == rm_define.anticlockwise:
                speed_yaw = -speed
            else:
                pass
    ...
    def _set_chassis_speed(self, speed_x, speed_y, speed_z, update_f_angle=False):
        logger.info('chassis move mode: ' + str(self.attr_dict['mode']))
    ...
    def update_position(self, x=None, y=None, yaw_angle=None, m_speed=None, r_speed=None, axis_mode=rm_define.axis_mode_default, wait_for_complete=True):
        ...
        if yaw_angle == None:
            yaw_angle = 0
        result = get_result((self._ChassisCtrl__update_position)(x, y, -yaw_angle, wait_for_complete=wait_for_complete, **kw))
        return result
...
class GunCtrl(RobotCtrlTool):
    ...
    def __init__(self, event_client, type='gel'):
        ...
        self.timer_count = 0
        self.count = 1
        self._GunCtrl__type = 0
        if type == 'gel':
            self._GunCtrl__type = 0
        else:
            if type == 'ir':
                self._GunCtrl__type = 1
    ...
    def _fire_timer_func(self, *arg, **kw):
        if self.timer_count == 0:
            self.gun.set_cmd_fire(self._GunCtrl__type, self.count)
        self.timer_count += 1
        self.timer_count %= 10
    ...
    def _stop_fire_timer(self):
        if self.gun_timer:
            if self.gun_timer.is_start():
                self.gun_timer.join()
                self.gun_timer.stop()

    def _destory_fire_timer(self):
        if self.gun_timer:
            if self.gun_timer.is_alive():
                self.gun_timer.join()
    ...
    def fire_once(self):
        ...
        duss_result = rm_define.DUSS_SUCCESS
        duss_result = self.gun.set_cmd_fire(self._GunCtrl__type, self.count)
        return get_task_dict(None, 'action_immediate', (), result=duss_result)
...
class GimbalCtrl(RobotCtrlTool):
    ...
    def __init__(self, event_client):
        ...
        self.event_client = event_client
        self.attr_dict = {'mode':rm_define.gimbal_free_mode, 
         'suspend_state':False, 
         'stick_overlay_enable':rm_define.stick_overlay_disable, 
         'pitch_acc':30, 
         'yaw_acc':30, 
    ...
    def set_rotate_speed(self, speed, speed2=None):
        ...
        if self.check_action_state('rotate'):
            ...
            if self.check_action_state('rotate_with_speed'):
    ...
    def _stop_rotate(self):
        ...
        if self.check_action_state('rotate') or self.check_action_state('rotate_with_speed'):
            duss_result = self.gimbal.set_accel_ctrl(0, 0, 0)
            if duss_result == rm_define.DUSS_SUCCESS:
                self.reset_action_state('rotate')
    ...
    def rotate(self, direction=None):
        ...
        CHECK_VALUE_IN_ENUM_LIST(direction, rm_define.gimbal_up=rm_define.gimbal_up, 
        ...
        if direction == rm_define.gimbal_up:
            ...
            if direction == rm_define.gimbal_down:
                ...
                if direction == rm_define.gimbal_right:
                    yaw_accel = self.attr_dict['yaw_acc']
                else:
                    if direction == rm_define.gimbal_left:
                        yaw_accel = -self.attr_dict['yaw_acc']
                    else:
                        pitch_accel = self.attr_dict['pitch_acc']
                        yaw_accel = self.attr_dict['yaw_acc']
    ...
    def rotate_with_degree(self, direction, degree):
        ...
        if direction == rm_define.gimbal_down or direction == rm_define.gimbal_up:
            ...
            if direction == rm_define.gimbal_left or direction == rm_define.gimbal_right:
        ...
        if self.attr_dict['mode'] == rm_define.gimbal_yaw_follow_mode:
            if axis_maskbit & rm_define.gimbal_axis_yaw_maskbit == 1:
        ...
        logger.info('axis_maskbit: ' + str(axis_maskbit))
        if duss_result == rm_define.DUSS_SUCCESS:
            return get_task_dict(identify, 'task', (direction, degree), result=duss_result)
        self.interrupt_func_unregister()
        return get_task_dict(None, 'no_task', (direction, degree), result=duss_result)
    ...
    def get_speed(self):
        return (
         self.attr_dict['pitch_acc'], self.attr_dict['yaw_acc'])
    ...
    def update_angle(self, pitch=None, yaw=None, pspeed=None, yspeed=None, coordinate=rm_define.gimbal_coodrdinate_4, wait_for_complete=True):
        ...
        result = get_result((self._GimbalCtrl__angle_ctrl)(yaw, pitch, wait_for_complete=wait_for_complete, **kw))
    ...
    def update_status(self, status):
        ...
        if status == rm_define.gimbal_status.wake:
            ...
            if status == rm_define.gimbal_status.sleep:
    ...
    def recenter(self, *xargs):
        if len(xargs) == 0:
            self._GimbalCtrl__recenter()
        else:
            if len(xargs) == 1:
                result = get_result(self._GimbalCtrl__recenter(xargs[0], xargs[0]))
                return result
            if len(xargs) == 2:
                result = get_result(self._GimbalCtrl__recenter(xargs[0], xargs[1]))
                return result
            return False
...
class LedCtrl(RobotCtrlTool):
    ...
    def set_led(self, comp, r, g, b, effect):
        CHECK_VALUE_IN_ENUM_LIST(comp, rm_define.armor_all=rm_define.armor_all, 
        ...
        CHECK_VALUE_IN_ENUM_LIST(effect, rm_define.effect_always_on=rm_define.effect_always_on, 
        ...
        if effect == rm_define.effect_always_on:
            ...
            if effect == rm_define.effect_always_off:
                duss_result = self.led.set_led(comp, 0, 7, r, g, b, 0, 1000, 1000)
            else:
                if effect == rm_define.effect_breath:
                    ...
                    if effect == rm_define.effect_flash:
                        ...
                        if effect == rm_define.effect_marquee:
    ...
    def set_top_led(self, comp, r, g, b, effect):
        ...
        CHECK_VALUE_IN_ENUM_LIST(comp, rm_define.armor_top_all=rm_define.armor_top_all, 
         rm_define.armor_top_right=rm_define.armor_top_right, 
         rm_define.armor_top_left=rm_define.armor_top_left)
        CHECK_VALUE_RANGE_AND_TYPE(r, 0, 255, int)
        CHECK_VALUE_RANGE_AND_TYPE(g, 0, 255, int)
    ...
    def set_bottom_led(self, comp, r, g, b, effect):
        ...
        CHECK_VALUE_IN_ENUM_LIST(comp, rm_define.armor_bottom_all=rm_define.armor_bottom_all, 
         rm_define.armor_bottom_back=rm_define.armor_bottom_back, 
         rm_define.armor_bottom_front=rm_define.armor_bottom_front, 
         rm_define.armor_bottom_left=rm_define.armor_bottom_left, 
         rm_define.armor_bottom_right=rm_define.armor_bottom_right)
    ...
    def set_single_led(self, comp, idx, effect, **kw):
        ...
        if type(idx) == list or isinstance(idx, rm_builtins.RmList):
            ...
            for i in idx:
                ...
                if i >= rm_define.armor_top_led_index_min:
                    if i <= rm_define.armor_top_led_index_max:
                        mask = mask | 1 << i - 1

        else:
            if type(idx) == int:
                CHECK_VALUE_RANGE(idx, rm_define.armor_top_led_index_min, rm_define.armor_top_led_index_max)
                if not idx >= rm_define.armor_top_led_index_min or idx <= rm_define.armor_top_led_index_max:
                    mask = 1 << idx - 1
            else:
    ...
    def update_led_t(self, comp, effect, r, g, b, blink_freq, single_led_index):
        attr_keys = {'all':rm_define.armor_all,  'top_all':rm_define.armor_top_all, 
...
class ArmorCtrl(RobotCtrlTool):
    ...
    def hit_event_process(self, event_client, msg):
        ...
        data = msg['data']
        self.hit_index = data[0] >> 4 & 15
        self.hit_type = data[0] & 15
        ...
        if self.hit_index == 1:
            if ArmorCtrl.ARMOR_BOTTOM_BACK_STR in self.hit_event_cb:
                self.condition_mutex.acquire()
        ...
        if self.hit_index == 2:
            if ArmorCtrl.ARMOR_BOTTOM_FRONT_STR in self.hit_event_cb:
                self.condition_mutex.acquire()
        ...
        if self.hit_index == 3:
            if ArmorCtrl.ARMOR_BOTTOM_LEFT_STR in self.hit_event_cb:
                self.condition_mutex.acquire()
        ...
        if self.hit_index == 4:
            if ArmorCtrl.ARMOR_BOTTOM_RIGHT_STR in self.hit_event_cb:
                self.condition_mutex.acquire()
        ...
        if self.hit_index == 5:
            if ArmorCtrl.ARMOR_TOP_LEFT_STR in self.hit_event_cb:
                self.condition_mutex.acquire()
        ...
        if self.hit_index == 6:
            if ArmorCtrl.ARMOR_TOP_RIGHT_STR in self.hit_event_cb:
                self.condition_mutex.acquire()
    ...
    def ir_event_process(self, event_client, msg):
        ...
        data = msg['data']
        self.ir_recv_dev = data[1] & 15
        if ArmorCtrl.IR_TOP_LEFT_STR in self.hit_event_cb:
            if self.ir_recv_dev & 1:
                self.condition_mutex.acquire()
        ...
        if ArmorCtrl.IR_TOP_RIGHT_STR in self.hit_event_cb:
            if self.ir_recv_dev & 2:
                self.condition_mutex.acquire()
        ...
        if ArmorCtrl.IR_ALL_STR in self.hit_event_cb:
            if self.ir_recv_dev:
                self.condition_mutex.acquire()
    ...
    def cond_wait(self, func_str):
        ...
        CHECK_VALUE_IN_ENUM_LIST(func_str, rm_define.cond_armor_hit=rm_define.cond_armor_hit, 
        ...
        self.robot_sleep(3600000, self.check_cond_wait_event, func_str)
    ...
    def get_last_hit_info(self):
        return (
         self.get_last_hit_index(), self.get_last_hit_time())
...
class VisionCtrl(RobotCtrlTool):
    event_detection_type_dict = {rm_define.vision_detection_people_type: {rm_define.detection_all_default: 'vision_recognized_people'}, 
     ...
     rm_define.vision_detection_pose_type: {rm_define.pose_all: 'vision_recognized_pose_all', 
     ...
     rm_define.vision_detection_marker_type: {rm_define.marker_all: 'vision_recognized_marker_all', 
    ...
    event_detection_mask_dict = {'vision_recognized_people':rm_define.vision_detection_people, 
    ...
    detection_enable_priority = {rm_define.vision_detection_auto_aim: {'core_num':rm_define.vision_core_num_10, 
                                           'priority':rm_define.vision_priority_auto_aim}, 
     
     rm_define.vision_detection_people: {'core_num':rm_define.vision_core_num_8, 
                                         'priority':rm_define.vision_priority_people}, 
     
     rm_define.vision_detection_pose: {'core_num':rm_define.vision_core_num_8, 
                                       'priority':rm_define.vision_priority_pose}, 
     
     rm_define.vision_detection_head_shoulder: {'core_num':rm_define.vision_core_num_8, 
                                                'priority':rm_define.vision_priority_head_shoulder}, 
     
     rm_define.vision_detection_marker: {'core_num':rm_define.vision_core_num_8, 
                                         'priority':rm_define.vision_priority_marker}, 
     
     rm_define.vision_detection_line: {'core_num':rm_define.vision_core_num_4, 
                                       'priority':rm_define.vision_priority_line}, 
     
     rm_define.vision_detection_people_follow: {'core_num':rm_define.vision_core_num_4, 
                                                'priority':rm_define.vision_priority_people_follow}, 
     
     rm_define.vision_detection_car: {'core_num':rm_define.vision_core_num_4, 
    ...
    def __init__(self, event_client):
        ...
        self.vision_detection_data = {'data':{rm_define.vision_detection_head_shoulder_type: 0, 
         ...
         'time':{rm_define.vision_detection_head_shoulder_type: 0, 
          rm_define.vision_detection_people_type: 0, 
          rm_define.vision_detection_pose_type: 0, 
          rm_define.vision_detection_auto_aim_type: 0, 
          rm_define.vision_detection_line_type: 0, 
    ...
    def __enable_detection(self, vision_func):
        ...
        CHECK_VALUE_IN_ENUM_LIST(vision_func, rm_define.vision_detection_marker=rm_define.vision_detection_marker, 
    ...
    def enable_detection(self, vision_func):
        result = get_result(self._VisionCtrl__enable_detection(vision_func))
        return result

    def disable_detection(self, vision_func):
        result = get_result(self._VisionCtrl__disable_detection(vision_func))
        return result
    ...
    def attr_set(self, line_color=None, marker_color=None, marker_dist=None):
        result = True
        line_color_enum = {'red':rm_define.line_color.red, 
         'yellow':rm_define.line_color.yellow, 
         'blue':rm_define.line_color.blue}
        marker_color_enum = {'red':rm_define.marker_color.red, 
                ...
                for i in range(1, num + 1):
                    ...
                    if self.sdk_info_push_callback:
                        ...
                        if detection_type == rm_define.vision_detection_people_type:
                            ...
                            if detection_type == rm_define.vision_detection_pose_type:
                                ...
                                if detection_type == rm_define.vision_detection_marker_type:
                                    ...
                                    if detection_type == rm_define.vision_detection_line_type:
                                        ...
                                        if detection_type == rm_define.vision_detection_car_type:
                                ...
                                while len(callback_data) != 0:
                                    for data_t in callback_data:
                                        ...
                                        if data_t.info in kind_callback_data.keys():
                                            ...
                                            kind_callback_data[data_t.info] = [
                                             data_t]
                                    else:
                                        for index in kind:
                                            if index in VisionCtrl.event_detection_type_dict[detection_type].keys():
                                                if VisionCtrl.event_detection_type_dict[detection_type][index] in self.recognition_event_cb:
    ...
    def register_event(self, func_dict):
        ...
        for event_str in VisionCtrl.event_detection_mask_dict.keys():
            if event_str in func_dict.keys():
                ...
                if event_str in VisionCtrl.event_detection_callback_need_data_dict.keys():
                    if VisionCtrl.event_detection_callback_need_data_dict[event_str]:
    ...
    def get_marker_xx_all_func_str(self, detection_type, index):
        if detection_type == rm_define.vision_detection_marker_type:
            if index >= rm_define.marker_number_zero:
                if index <= rm_define.marker_number_nine:
                    return VisionCtrl.event_detection_type_dict[detection_type][rm_define.marker_number_all]
            if index >= rm_define.marker_letter_A:
                if index <= rm_define.marker_letter_Z:
                    return VisionCtrl.event_detection_type_dict[detection_type][rm_define.marker_letter_all]
            if index >= rm_define.marker_trans_left:
                if index <= rm_define.marker_trans_backward:
                    return VisionCtrl.event_detection_type_dict[detection_type][rm_define.marker_trans_all]
            if index >= rm_define.marker_letter_plus:
                if index <= rm_define.marker_letter_divide:
                    return VisionCtrl.event_detection_type_dict[detection_type][rm_define.marker_trans_all]
            return
        else:
            return
        ...
        for item in data:
            ...
            if self.is_line_there:
                ...
                if cur_time - self.line_there_last_time > self.line_lost_time_out:
                    if self.line_lost_first_chassis_stop_flag:
                        if not self.line_lost_has_ctrl or self.line_follow_auto_ctrl_flag:
    ...
    @event_register
    def start_line_follow_until_exception--- This code section failed: ---

 L.3349         0  LOAD_GLOBAL              logger
                2  LOAD_METHOD              info
                4  LOAD_STR                 'VISION_CTRL: start line follow until exception,  the line color is %s'
                6  LOAD_GLOBAL              color
                8  BINARY_MODULO    
               10  CALL_METHOD_1         1  ''
               12  POP_TOP          

 L.3350        14  BUILD_LIST_0          0 
               16  LOAD_FAST                'self'
               18  STORE_ATTR               cur_marker_with_follow_line_list

 L.3351        20  LOAD_CONST               None
               22  LOAD_FAST                'self'
               24  STORE_ATTR               line_intersection

 L.3352        26  LOAD_FAST                'self'
               28  LOAD_ATTR                line_detection_flag
               30  LOAD_CONST               False
               32  COMPARE_OP               ==
               34  POP_JUMP_IF_FALSE   112  'to 112'

 L.3353        36  LOAD_FAST                'self'
               38  LOAD_METHOD              start_line_detection
               40  CALL_METHOD_0         0  ''
               42  STORE_FAST               'result'

 L.3354        44  LOAD_FAST                'result'
               46  LOAD_GLOBAL              rm_define
               48  LOAD_ATTR                DUSS_SUCCESS
               50  COMPARE_OP               ==
               52  POP_JUMP_IF_FALSE   106  'to 106'

 L.3355        54  LOAD_FAST                'self'
               56  LOAD_ATTR                line_follow_auto_ctrl_thread
               58  LOAD_CONST               None
               60  COMPARE_OP               ==
               62  POP_JUMP_IF_TRUE     74  'to 74'
               64  LOAD_FAST                'self'
               66  LOAD_ATTR                line_follow_auto_ctrl_thread
               68  LOAD_METHOD              isAlive
               70  CALL_METHOD_0         0  ''
               72  POP_JUMP_IF_TRUE    106  'to 106'
             74_0  COME_FROM            62  '62'

 L.3356        74  LOAD_GLOBAL              threading
               76  LOAD_ATTR                Thread
               78  LOAD_FAST                'self'
               80  LOAD_ATTR                line_follow_auto_ctrl
               82  LOAD_CONST               ('target',)
               84  CALL_FUNCTION_KW_1     1  '1 total positional and keyword args'
               86  LOAD_FAST                'self'
               88  STORE_ATTR               line_follow_auto_ctrl_thread

 L.3357        90  LOAD_FAST                'self'
               92  LOAD_ATTR                line_follow_auto_ctrl_thread
               94  LOAD_METHOD              start
               96  CALL_METHOD_0         0  ''
               98  POP_TOP          

 L.3358       100  LOAD_CONST               True
              102  LOAD_FAST                'self'
              104  STORE_ATTR               line_follow_auto_ctrl_flag
            106_0  COME_FROM            72  '72'
            106_1  COME_FROM            52  '52'

 L.3359       106  LOAD_CONST               True
              108  LOAD_FAST                'self'
              110  STORE_ATTR               line_detection_flag
            112_0  COME_FROM            34  '34'

 L.3360       112  LOAD_FAST                'self'
              114  LOAD_ATTR                vision
              116  LOAD_METHOD              detection_attr_set
              118  LOAD_CONST               1
              120  LOAD_FAST                'self'
              122  LOAD_ATTR                line_follow_color
              124  CALL_METHOD_2         2  ''
              126  POP_TOP          

 L.3361       128  LOAD_CONST               False
              130  LOAD_FAST                'self'
              132  STORE_ATTR               line_follow_pause_flag
            134_0  COME_FROM           152  '152'
            134_1  COME_FROM           148  '148'

 L.3363       134  LOAD_FAST                'self'
              136  LOAD_METHOD              _wait
              138  LOAD_CONST               0.2
              140  CALL_METHOD_1         1  ''
              142  POP_TOP          

 L.3364       144  LOAD_FAST                'self'
              146  LOAD_ATTR                line_follow_exception
              148  POP_JUMP_IF_FALSE_LOOP   134  'to 134'

 L.3365       150  JUMP_FORWARD        154  'to 154'
              152  JUMP_LOOP           134  'to 134'
            154_0  COME_FROM           150  '150'

 L.3366       154  LOAD_CONST               False
              156  LOAD_FAST                'self'
              158  STORE_ATTR               line_follow_exception

 L.3367       160  LOAD_GLOBAL              logger
              162  LOAD_METHOD              info
              164  LOAD_STR                 'VISION_CTRL: line follow Exception'
              166  CALL_METHOD_1         1  ''
              168  POP_TOP          

 L.3368       170  LOAD_GLOBAL              get_task_dict
              172  LOAD_CONST               None
              174  LOAD_STR                 'action_immediate'
              176  LOAD_FAST                'self'
              178  LOAD_ATTR                line_follow_color
              180  BUILD_MAP_0           0 
              182  CALL_FUNCTION_4       4  ''
              184  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `JUMP_LOOP' instruction at offset 152

    def line_follow_auto_ctrl(self):
        while True:
            if self.line_follow_auto_ctrl_flag:
                if not self.line_follow_pause_flag:
                    if self.is_line_there:
    ...
    def line_follow_chassis_ctrl(self, direction):
        ...
        if direction == rm_define.line_follow_chassis_stop:
            ...
            pass
        if direction == rm_define.line_follow_chassis_right or direction == rm_define.line_follow_chassis_left or direction == rm_define.line_follow_chassis_front:
            self.line_follow_line_choice(direction)
        else:
            if direction == rm_define.line_follow_chassis_turn_around:
                self.line_follow_chassis_ctrl(rm_define.line_follow_chassis_stop)
    ...
    def line_follow_color_set(self, color):
        CHECK_VALUE_IN_ENUM_LIST(color, rm_define.line_follow_color_red=rm_define.line_follow_color_red, 
    ...
    def marker_detection_color_set(self, color):
        CHECK_VALUE_IN_ENUM_LIST(color, rm_define.marker_detection_color_red=rm_define.marker_detection_color_red, 
    ...
    def line_follow_line_choice(self, direction):
        ...
        if direction == rm_define.line_follow_chassis_left:
            return self.vision.detection_attr_set(2, 2)
        if direction == rm_define.line_follow_chassis_right:
    ...
    def get_line_detection_deviation_point(self, point=rm_define.line_detection_near_point):
        ...
        if point == rm_define.line_detection_middle_point:
        ...
        while True:
            if run_time < 5:
                ...
                for item in result_info:
                    if item.type == rm_define.vision_detection_marker_type:
                        while item.info == marker_id:
                            offset_pitch = (0.53 - item.pos.y) * 54
                            offset_yaw = (item.pos.x - 0.5) * 96
                            if abs(offset_pitch) < 1:
                                if abs(offset_yaw) < 1:
                                    gimbal_yaw_ctrl = {
                                      'amount': 0, 'kp': 0, 'ki': 0, 'kd': 0, 'max_speed': 0}
                                    gimbal_pitch_ctrl = { 'amount': 0, 'kp': 0, 'ki': 0, 'kd': 0, 'max_speed': 0}
                                    self.set_line_follow_param_gimbal(rm_define.robot_mode_chassis_follow, gimbal_yaw_ctrl, gimbal_pitch_ctrl)
                                    return get_task_dict(None, 'action_immediate', (marker_id,), {})
                            gimbal_yaw_ctrl = {'amount':item.pos.x - 0.5, 
                             'kp':1800,  'ki':0,  'kd':1.5,  'max_speed':250}
    ...
    def get_vision_detection_info(self, detection_type):
        ...
        if time.time() - cur_time > 1:
            ...
            while len(data_list) > 0:
                while detection_type == rm_define.vision_detection_line_type:
                    if detection_type == data_list[0].type:
                        ...
                        while not detection_type == rm_define.vision_detection_people_type:
                            if not detection_type == rm_define.vision_detection_car_type or detection_type == data_list[0].type:
                                ...
                                while detection_type == data_list[0].type:
                                    output_list[0] = len(data_list)
                                    for item in data_list:
...
class MediaCtrl(RobotCtrlTool):
    event_detection_type_dict = {rm_define.sound_detection_applause_type: {rm_define.applause_all: 'sound_recognized_applause_all', 
    ...
    event_detection_mask_dict = {'sound_recognized_applause_all':rm_define.sound_detection_applause, 
    ...
    event_detection_callback_need_data_dict = {'sound_recognized_applause_all': True}
    ...
    def register_event(self, func_dict):
        ...
        for event_str in MediaCtrl.event_detection_mask_dict.keys():
            if event_str in func_dict.keys():
                ...
                if event_str in MediaCtrl.event_detection_callback_need_data_dict.keys():
                    if MediaCtrl.event_detection_callback_need_data_dict[event_str]:
                        self.wait_event_callback_need_data_set_add(event_str)
        else:
    ...
    def enable_sound_recognition(self, func_mask):
        CHECK_VALUE_IN_ENUM_LIST(func_mask, **{'rm_define.sound_detection_applause': rm_define.sound_detection_applause})
    ...
    def cond_wait(self, func_str):
        ...
        CHECK_VALUE_IN_ENUM_LIST(func_str, rm_define.cond_sound_recognized_applause_twice=rm_define.cond_sound_recognized_applause_twice, 
    ...
    def check_condition(self, recog_type):
        CHECK_VALUE_IN_ENUM_LIST(recog_type, rm_define.cond_sound_recognized_applause_twice=rm_define.cond_sound_recognized_applause_twice, 
         rm_define.cond_sound_recognized_applause_thrice=rm_define.cond_sound_recognized_applause_thrice)
        self.condition_mutex.acquire()
        event_happen = False
    ...
    def play_lib_sound(self, id, wait_for_complete_flag=False):
        ...
        if wait_for_complete_flag == True:
            self.interrupt_func_register(self.media.play_sound_task_stop)
            duss_result, identify = self.media.play_sound_task(240 + id)
            if duss_result == rm_define.DUSS_SUCCESS:
                return get_task_dict(identify, 'task', (id, wait_for_complete_flag), result=duss_result)
            self.interrupt_func_unregister()
            return get_task_dict(None, 'no_task', (id, wait_for_complete_flag), result=duss_result)
        else:
            duss_result = self.media.play_sound(240 + id)
            return get_task_dict(None, 'action_immediate', (id, wait_for_complete_flag), result=duss_result)

    @event_register
    def exposure_value_update(self, ev):
        CHECK_VALUE_IN_ENUM_LIST(ev, rm_define.exposure_value_default=rm_define.exposure_value_default, 
    ...
    def play_sound(self, id, **kw):
        CHECK_VALUE_IN_ENUM_LIST(id, rm_define.media_sound_attacked=rm_define.media_sound_attacked, 
        ...
        return get_result(self._MediaCtrl__play_sound(id, wait_for_complete))
...
class RobotCtrl(object):
    ...
    def set_mode(self, mode):
        ...
        if not mode == rm_define.robot_mode_gimbal_follow:
            if mode == rm_define.robot_mode_chassis_follow or mode == rm_define.robot_mode_free:
    ...
    def get_mode(self):
        if self.mode == rm_define.robot_mode_gimbal_follow:
            return 'chassis_lead'
        if self.mode == rm_define.robot_mode_chassis_follow:
            return 'gimbal_lead'
        if self.mode == rm_define.robot_mode_free:
            return 'free'
    ...
    def sub_gimbal_and_chassis_info(self, uuid_list, callback):
        ...
        if duss_result == rm_define.DUSS_SUCCESS:
            if resp['data'][0] == 0 or resp['data'][0] == 35:
                logger.info('TANK_CTRL: sub gimbal and chassis info')
    ...
    def gimbal_and_chassis_push_info_process(self, event_client, msg):
        ...
        self.gimbal_status_dict['stick_flag'] = stick_flag >> 1 & 1
        self.chassis_status_dict['stick_flag'] = stick_flag & 1 | self.gimbal_status_dict['stick_flag']
        ...
        servo_id = servo_info[0] & 15
        for i in range(servo_id.bit_length()):
            if servo_id >> i & 1 == 1:
                self.servo_status_dict[i + 1] = {'speed':tools.byte_to_int16(servo_info[i * 2 + 1:i * 2 + 3]),  'angle':tools.byte_to_int16(servo_info[i * 2 + 9:i * 2 + 11])}
        else:
...
class MobileCtrl(object):
    ...
    def __init__(self, event_client):
        ...
        self.mobile_type_info_parse = {rm_define.mobile_info_accel_type: self.accel_data_parse, 
         rm_define.mobile_info_atti_type: self.atti_data_parse, 
         rm_define.mobile_info_gyro_type: self.gyro_data_parse}
        self.mobile_accel_data = [
         0, 0, 0]
    ...
    def atti_data_parse(self, data):
        ...
        self.mobile_atti_data = [
         atti_pitch, atti_roll, atti_yaw]
...
class LogCtrl(object):
    ...
    def _add_string_head(self, level, string):
        string_head = {rm_define.custom_msg_level_none: '', 
...
class RobotTools(RobotCtrlTool):
    ...
    def timer_ctrl(self, ctrl):
        CHECK_VALUE_IN_ENUM_LIST(ctrl, rm_define.timer_start=rm_define.timer_start, 
        ...
        if ctrl == rm_define.timer_start:
            ...
            pass
        if ctrl == rm_define.timer_stop:
            if self.timer_state == rm_define.timer_start:
                self.timer_total = self.timer_total + (time.time() - self.timer_start_stamp)
                self.timer_start_stamp = 0
                self.timer_state = rm_define.timer_stop
        else:
            if ctrl == rm_define.timer_reset and self.timer_state != rm_define.timer_reset:
                self.timer_start_stamp = 0
                self.timer_total = 0
    ...
    def get_localtime(self, time_unit):
        CHECK_VALUE_IN_ENUM_LIST(time_unit, rm_define.localtime_year=rm_define.localtime_year, 
        ...
        if time_unit == rm_define.localtime_month:
            return time.localtime().tm_mon
        if time_unit == rm_define.localtime_day:
            return time.localtime().tm_mday
        if time_unit == rm_define.localtime_hour:
            return time.localtime().tm_hour
        if time_unit == rm_define.localtime_minute:
            return time.localtime().tm_min
        if time_unit == rm_define.localtime_second:
...
class ModulesStatusCtrl(object):
    ...
    EXTEND_MODULE_ID = (
     ...
     rm_define.robotic_gripper_id,
     rm_define.robotic_arm_id)
    MODULE_STATUS_MSG_ID_TO_MODULE_ID_MAP = {256:rm_define.system_id, 
     260:rm_define.camera_id, 
     768:rm_define.chassis_id, 
     1024:rm_define.gimbal_id, 
     5888:rm_define.gun_id, 
     6145:rm_define.armor1_id, 
     6146:rm_define.armor2_id, 
     6147:rm_define.armor3_id, 
     6148:rm_define.armor4_id, 
     6149:rm_define.armor5_id, 
     6150:rm_define.armor6_id, 
     5633:rm_define.sensor_adapter1_id, 
     5634:rm_define.sensor_adapter2_id, 
     5635:rm_define.sensor_adapter3_id, 
     5636:rm_define.sensor_adapter4_id, 
     5637:rm_define.sensor_adapter5_id, 
     5638:rm_define.sensor_adapter6_id, 
     4609:rm_define.tof1_id, 
     4610:rm_define.tof2_id, 
     4611:rm_define.tof3_id, 
     4612:rm_define.tof4_id, 
     792:rm_define.servo1_id, 
     793:rm_define.servo2_id, 
     794:rm_define.servo3_id, 
     795:rm_define.servo4_id, 
     798:rm_define.robotic_arm_id, 
     799:rm_define.robotic_gripper_id}
    ...
    def check_module_invalid(self, event_msg):
        if self._check_is_edu_module(event_msg.module_id):
            if not self.is_edu_version:
                ...
                return (
                 True, rm_define.FAT_DEVICE_NOT_SUPPORT)
            ...
            if self._check_module_status(event_msg.module_id, rm_define.module_status_error):
    ...
    def event_init(self):
        ...
        for id in range(1, 8):
            ...
            for event_name in self.pulse_event_cb:
                ...
                resp = (self.sensor_adapter.set_sensor_adapter_param)(*(0, 1), **)
                (self.sensor_adapter.set_sensor_adapter_param)(*(0, 2), **)
        ...
        for board in range(1, 8):
            for port in range(1, 3):
                ...
                if board_id == board:
                    if port_num == port:
                        if io_value == 1:
                            if func_str in self.pulse_event_cb:
                                self.condition_mutex.acquire()
                                self._wakeup_condition_waiting(func_str)
                                self.condition_event_time[func_str] = time.time()
                                self.condition_event_table[func_str] = True
    ...
    def check_condition(self, func_str):
        ...
        if func_str in self.condition_event_table.keys():
            ...
            if self.get_sensor_adapter_io_level(adapterid, portid) == 1:
                ...
                if self.get_sensor_adapter_io_level(adapterid, portid) == 0:
                    if 'high' in func_str:
                        self.condition_event_table[func_str] = False
                    else:
                        if 'low' in func_str:
...
class SpeechRecognizerCtrl(RobotCtrlTool):
    __doc__ = 'docstring for SpeechRecognizerCtrl'
    ...
    def __init__(self, event_client):
        ...
        self.volume = 0
        self.event_client = event_client
        self.speech_recognizer = rm_module.SpeechRecognizer(event_client)

    @event_register
    def enable_speech_volume_check(self):
        duss_result = rm_define.DUSS_SUCCESS
        duss_result = self.speech_recognizer.speech_check_ctrl(1)
        if duss_result == rm_define.DUSS_SUCCESS:
            self.speech_recognizer.sub_speech_data_push(self.speech_push_info)
        return get_task_dict(None, 'action_immediate', (), result=duss_result)

    @event_register
    def disable_speech_volume_check(self):
        duss_result = rm_define.DUSS_SUCCESS
        duss_result = self.speech_recognizer.speech_check_ctrl(0)
        return get_task_dict(None, 'action_immediate', (), result=duss_result)

    @event_register
    def start_speech_recognizer_with_time(self, lan_type, wait_time):
        logger.info('Speech Recognizer: Start, type is ' + str(lan_type) + ' time is ' + str(wait_time))
        CHECK_VALUE_RANGE_AND_TYPE(lan_type, 1, 3, int, float)
        CHECK_VALUE_RANGE_AND_TYPE(wait_time, 1, 5, int, float)
        duss_result = rm_define.DUSS_SUCCESS
        self.recognizer_result = ''
        self.speech_recognizer.sub_speech_data_push(self.speech_push_info)
        duss_result, resp = self.speech_recognizer.speech_recognizer_start_with_time(lan_type, wait_time)
        if resp:
            if resp['data'][0] != 0:
                err_code = tools.byte_to_int32(resp['data'][2:6])
                err_code = self.get_error_code(err_code)
                return get_task_dict(None, 'action_with_time', (), result=duss_result, err_code=err_code)
        return get_task_dict(None, 'action_with_time', (lan_type, wait_time), result=duss_result)

    @event_register
    def start_text_to_speech(self, role_name, volume, speech_rate, pitch_rate, speech_text, wait_for_complete_flag=False):
        logger.info('SpeechRecognizerCtrl: start_text_to_speech : %s, wait_for_complete_flag is %s' % (speech_text, wait_for_complete_flag))
        CHECK_VALUE_RANGE_AND_TYPE(volume, 0, 100, int, float)
        CHECK_VALUE_RANGE_AND_TYPE(speech_rate, -500, 500, int, float)
        CHECK_VALUE_RANGE_AND_TYPE(pitch_rate, -500, 500, int, float)
        if wait_for_complete_flag == True:
            duss_result, identify, resp = self.speech_recognizer.play_text_to_speech_task(role_name, volume, speech_rate, pitch_rate, speech_text)
            if duss_result == rm_define.DUSS_SUCCESS:
                return get_task_dict(identify, 'task', (role_name, wait_for_complete_flag), result=duss_result)
            if resp:
                if resp['data'][0] != 0:
                    err_code = tools.byte_to_int32(resp['data'][2:6])
                    err_code = self.get_error_code(err_code)
                    return get_task_dict(None, 'action_immediate', (), result=duss_result, err_code=err_code)
            return get_task_dict(None, 'no_task', (role_name, wait_for_complete_flag), result=duss_result)
        else:
            duss_result, resp = self.speech_recognizer.speech_text_to_speech_start(role_name, volume, speech_rate, pitch_rate, speech_text)
            if resp:
                if resp['data'][0] != 0:
                    err_code = tools.byte_to_int32(resp['data'][2:6])
                    err_code = self.get_error_code(err_code)
                    return get_task_dict(None, 'action_immediate', (), result=duss_result, err_code=err_code)
        return get_task_dict(None, 'action_immediate', (role_name, wait_for_complete_flag), result=duss_result)

    @event_register
    def stop_text_to_speech(self):
        duss_result = rm_define.DUSS_SUCCESS
        duss_result = self.speech_recognizer.stop_text_to_speech()
        return get_task_dict(None, 'action_immediate', (), result=duss_result)
    ...
    def exit(self):
        self.disable_speech_volume_check()

    def stop(self):
        pass

    def speech_push_info(self, event_client, msg):
        data = msg['data']
        cmd_id = data[0]
        if cmd_id == 1:
            self.volume = data[2] << 8 | data[1]
            logger.info('speech_push_info volume:%d' % self.volume)
        else:
            if cmd_id == 2:
                len = data[2] << 8 | data[1]
                tempdata = tools.pack_to_byte(data[3:len + 3])
                self.recognizer_result = tempdata.decode('utf-16le', errors='ignore')
                logger.info('识别结果:' + self.recognizer_result)

    def get_volume(self):
        return self.volume

    def get_recognizer_result(self):
        logger.info('get_recognizer_result: %s ' % self.recognizer_result)
        return self.recognizer_result

    def get_error_code(self, err_code):
        logger.info('SpeechRecognizerCtrl: get_error_code : %s ' % err_code)
        if err_code == -1:
            err_code = rm_define.BLOCK_ERR_SPEECH_NET_ERROR
        else:
            if err_code == -6 or err_code == -28:
                err_code = rm_define.BLOCK_ERR_SPEECH_INVALID_USER
            else:
                if err_code == -64 or err_code == -65:
                    err_code = rm_define.BLOCK_ERR_SPEECH_LIMIT
                else:
                    err_code = rm_define.BLOCK_ERR_SPEECH_OTHER
        return err_code
...
class IrDistanceSensorCtrl(RobotCtrlTool):
    ...
    def tof_push_info_process(self, event_client, msg):
        ...
        while port_id in self.event_callback_attr.keys():
    ...
    def _wakeup_condition_waitting(self, port_id, distance):
        while port_id in self.condition_wait_event_dict.keys():
    ...
    def register_event(self, func_dict):
        for key, value in func_dict.items():
            if re.match('^ir_distance_[1-9]{1}?_(lt|le|eq|ge|gt){1}?_[0-9]*_{0,1}?[0-9]+_event$', key):
                if callable(value):
    ...
    def cond_wait(self, func_str):
        if re.match('^ir_distance_[1-9]{1}?_(lt|le|eq|ge|gt){1}?_[0-9]*_{0,1}?[0-9]+$', func_str):
            condition_wait_event = threading.Event()
            port_id, comp, dist = self._parse_data(func_str)
            t_d = {'id':port_id,  'comp':comp,  'dist':dist}
            self._cond_wait_register(t_d, condition_wait_event)
            self.robot_sleep(3600000, self.check_cond_wait_event, t_d)
    ...
    def check_cond_wait_event(self, func_dict):
        ...
        if func_dict['id'] in self.condition_wait_event_dict.keys():
            for item in self.condition_wait_event_dict[func_dict['id']]:
                if item['attr']['comp'] == func_dict['comp']:
                    if item['attr']['dist'] == func_dict['dist']:
    ...
    def _cond_wait_unregister(self, func_dict):
        self.condition_mutex.acquire()
        while func_dict['id'] in self.condition_wait_event_dict.keys():
            for item in self.condition_wait_event_dict[func_dict['id']]:
                if item['attr']['comp'] == func_dict['comp']:
                    if item['attr']['dist'] == func_dict['dist']:
    ...
    def _comp(self, comp_str, param1, param2):
        if not (comp_str == 'lt' and param1 < param2):
            if comp_str == 'le' and not param1 <= param2:
                if comp_str == 'eq' and not param1 == param2:
                    if comp_str == 'ge' and not param1 >= param2:
                        if not comp_str == 'gt' or param1 > param2:
                            return True
            return False
...
class ServoCtrl(RobotCtrlTool):
    ...
    def __init__(self, event_client):
        ...
        self.servo_status = {}
        self.event_client = event_client
        self.mode = {1:rm_define.servo_unknow_mode, 
    ...
    def __switch_mode(self, servo_id, mode):
        ...
        if tools.duss_result_check(duss_result, resp, duml_cmdset.DUSS_MB_RET_OK) == rm_define.SUCCESS:
            ...
            return get_task_dict(None, 'action_immediate', (servo_id, mode), result=(rm_define.SUCCESS))
        if tools.duss_result_check(duss_result, resp, duml_cmdset.DUSS_MB_RET_SERVO_IN_ROBOTICARM_MODE) == rm_define.SUCCESS:
    ...
    def set_angle(self, servo_id, angle, wait_for_complete=False):
        ...
        if get_result(self._ServoCtrl__switch_mode(servo_id, rm_define.servo_position_mode)):
            if wait_for_complete == None:
                wait_for_complete = False
    ...
    def set_speed(self, servo_id, speed):
        if get_result(self._ServoCtrl__switch_mode(servo_id, rm_define.servo_speed_mode)):
...
class RoboticArmCtrl(RobotCtrlTool):
    ...
    def __init__(self, event_client):
        ...
        self.event_client = event_client
        self._RoboticArmCtrl__err_code = {3: 72}

    def __fail_reason_to_block_running_result(self, err_code):
        if err_code in self._RoboticArmCtrl__err_code.keys():
            return self._RoboticArmCtrl__err_code[err_code]
        return 0
    ...
    def move(self, x, y, wait_for_complete=False):
        ...
        if wait_for_complete == True:
            ...
            if duss_result == rm_define.DUSS_SUCCESS:
                return get_task_dict(identify, 'task', (x, y, wait_for_complete), result=duss_result, fail_reason_to_block_running_result_callback=(self._RoboticArmCtrl__fail_reason_to_block_running_result))
            self.interrupt_func_unregister()
    ...
    def recenter(self, wait_for_complete=False):
        ...
        if wait_for_complete == True:
            ...
            if duss_result == rm_define.DUSS_SUCCESS:
                return get_task_dict(identify, 'task', wait_for_complete, result=duss_result, fail_reason_to_block_running_result_callback=(self._RoboticArmCtrl__fail_reason_to_block_running_result))
            self.interrupt_func_unregister()
...
class SerialCtrl(RobotCtrlTool):
    ...
    def event_init(self):
        ...
        self.serial.set_serial_param(self.baud_rate, self.data_bit, self.odd_even, self.stop_bit, self.rx_enable, self.tx_enable, self.rx_buffer, self.tx_buffer)
    ...
    def serial_recv_process(self, event_client, msg):
        ...
        data = msg['data']
        msg_type = data[0]
        data_len = tools.byte_to_int16(data[1:3])
    ...
    def serial_config(self, baud_rate, data_bit, odd_even, stop_bit):
        ...
        self.serial.set_serial_param(baud_rate, data_bit, odd_even, stop_bit, self.rx_enable, self.tx_enable, self.rx_buffer, self.tx_buffer)
    ...
    def stop(self):
        ...
        self.serial.set_serial_param(self.baud_rate, self.data_bit, self.odd_even, self.stop_bit, 0, 0, self.rx_buffer, self.tx_buffer)
...
class TackLineCtrl(RobotCtrlTool):

    def __init__(self, event_client):
        super().__init__(event_client)
        self.trackline = rm_module.TrackLine(event_client)
        self.event_client = event_client

    @event_register
    def set_enable(self, ctrl):
        logger.info('TackLineCtrl: set_enable, ctrl is ' + str(ctrl))
        duss_result = rm_define.DUSS_SUCCESS
        duss_result = self.trackline.set_enable(ctrl)
        return get_task_dict(None, 'action_immediate', ctrl, result=duss_result)
