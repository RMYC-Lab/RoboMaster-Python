...
# Embedded file name: .\src\test\rm_module.py
# Compiled at: 2022-01-12 15:45:07
# Size of source mod 2**32: 87834 bytes
import random, threading, tools, rm_define, duml_cmdset, duss_event_msg, operator, rm_log
...
class Chassis(object):
    ...
    def set_work_mode(self, mode):
        logger.info('Chassis: set_work_mode, the mode is %s' % mode)
        self.msg_buff.init()
    ...
    def set_follow_speed(self, speed_x, speed_y, angle):
        logger.info('set_follow_speed: speed_x = ' + str(speed_x) + ', speed_y = ' + str(speed_y) + ', angle = ' + str(angle))
        if self.speed_limit_flag:
            speed_x = tools.data_limit(speed_x, -1, 1)
            speed_y = tools.data_limit(speed_y, -1, 1)
    ...
    def set_position_cmd_stop(self):
        ...
        self.msg_buff.append('vel_xy_max', 'uint8', 0)
        self.msg_buff.append('agl_omg_max', 'int16', 0)
        self.msg_buff.cmd_id = duml_cmdset.DUSS_MB_CMD_RM_POSITION_SET
...
class Gimbal(object):
    ...
    def set_work_mode(self, mode):
        logger.info('Gimbal: set_work_mode, the mode is %s' % mode)
        self.msg_buff.init()
        self.msg_buff.append('mode', 'uint8', mode)
    ...
    def set_stick_overlay(self, enable):
        if enable == rm_define.stick_overlay_enable:
            self.accel_contral_flag = self.accel_contral_flag & -65
        else:
            self.accel_contral_flag = self.accel_contral_flag | 64
    ...
    def return_middle(self, axis_maskbit, max_yaw_accel, max_roll_accel, max_pitch_accel):
        ...
        self.msg_buff.append('task_id', 'uint8', self.task_id)
        self.msg_buff.append('task_ctrl', 'uint8', task_ctrl)
        self.msg_buff.append('axis_maskbit', 'uint8', axis_maskbit)
    ...
    def set_degree_ctrl(self, pitch_degree, yaw_degree, pitch_accel, yaw_accel, axis_maskbit, coodrdinate, deviation=rm_define.gimbal_deviation, cmd_type=rm_define.NO_TASK):
        ...
        self.msg_buff.append('task_id', 'uint8', self.task_id)
        self.msg_buff.append('task_ctrl', 'uint8', task_ctrl)
        ctrl = (coodrdinate & 7) << 3 | axis_maskbit
    ...
    def set_compound_motion_ctrl(self, test_flag, enable_flag, axis, phase, cycle, margin, times):
        ctrl_flag = 0
        ctrl_flag = ctrl_flag | test_flag | enable_flag << 1 | axis << 2 | phase << 6 | 128
        margin = tools.data_limit(margin, 0, 2700)
        self.msg_buff.init()
        self.msg_buff.append('id', 'uint8', 13)
    ...
    def set_suspend_resume(self, state):
        ...
        if state == rm_define.gimbal_suspend:
            ...
            if state == rm_define.gimbal_resume:
                cmd = 32498
        self.msg_buff.init()
    ...
    def sub_attitude_info(self, callback):
        ...
        self.msg_buff.append('other', 'uint8', 255)
...
class Led(object):
    ...
    def _update_states(self, comp, pos_list, update_state):
        ...
        for pos in pos_list:
            ...
            return (
             control_mask, led_state)

    def _update_top_states(self, comp, ctrl):
        if comp & rm_define.armor_top_left:
            if ctrl == 0:
                self.led_state[rm_define.armor_top_left]['mask'] = 0
            else:
                self.led_state[rm_define.armor_top_left]['mask'] = 255
        if comp & rm_define.armor_top_right:
            if ctrl == 0:
                self.led_state[rm_define.armor_top_right]['mask'] = 0
            else:
                self.led_state[rm_define.armor_top_right]['mask'] = 255

    def _send_led_cmd(self, comp, led_state, **kw):
        ctrl_mode = (led_state['ctrl'] << 4 & 240) + (led_state['mode'] & 15)
        r = led_state['r']
        g = led_state['g']
        b = led_state['b']
        if 'r' in kw.keys():
    ...
    def set_gun_led(self, mode, enable_flag):
        self.msg_buff.init()
        enable_flag &= 1
        ctrl_mode = mode << 4 & 240 | enable_flag
        self.msg_buff.append('led_idx', 'uint32', 64)
        self.msg_buff.append('led_mask', 'uint16', 255)
        self.msg_buff.append('ctrl_mode', 'uint8', ctrl_mode)
        self.msg_buff.append('r', 'uint8', 255)
        self.msg_buff.append('g', 'uint8', 255)
        self.msg_buff.append('b', 'uint8', 255)
...
class Armor(object):
    ...
    def set_hit_sensitivity(self, k):
        ...
        self.msg_buff.init()
        self.msg_buff.append('set_type', 'uint8', 63)
...
class Media(object):
    ...
    def set_camera_zv(self, zv):
        self.msg_buff.init()
        self.msg_buff.append('mode', 'uint8', 9)
        self.msg_buff.append('oz', 'uint16', 0)
    ...
    def play_sound(self, id):
        ctrl = 1
        if id >= rm_define.media_sound_solmization_1C:
            if id <= rm_define.media_sound_solmization_3B:
                ctrl = 2
        self.soundid = id
        self.msg_buff.init()
        ...
        duss_result, resp = self.event_client.send_sync(self.msg_buff)
        if duss_result != rm_define.DUSS_SUCCESS:
            if id >= rm_define.media_custom_audio_0:
                if id <= rm_define.media_custom_audio_9:
    ...
    def play_sound_task(self, id):
        ...
        duss_result, identify = self.event_client.send_task_async(self.msg_buff, event_task)
        if duss_result != rm_define.DUSS_SUCCESS:
            if not id >= rm_define.media_custom_audio_0 or id <= rm_define.media_custom_audio_9:
                raise Exception('SCRIPT_CTRL: play rm_define.media_custom_audio_%d failed, raise exception' % (id - rm_define.media_custom_audio_0))
...
class Mobile(object):
    ...
    def unsub_all_info(self):
        self.msg_buff.init()
        self.msg_buff.append('info_id', 'uint16', 0)
        self.msg_buff.cmd_id = duml_cmdset.DUSS_MB_CMD_RM_SUB_MOBILE_INFO
        duss_result, resp = self.event_client.send_sync(self.msg_buff)
        if duss_result == duml_cmdset.DUSS_MB_RET_OK:
            cmd_set_id = duml_cmdset.DUSS_MB_CMDSET_RM << 8 | duml_cmdset.DUSS_MB_CMD_RM_MOBILE_INFO_PUSH
...
class Tank(object):
    ...
    def set_work_mode(self, mode):
        logger.info('Tank: set_work_mode, the mode is %s' % mode)
        self.msg_buff.init()
        self.msg_buff.append('mode', 'uint8', mode)
    ...
    def set_sdk_mode(self, enable):
        logger.info('set_sdk_mode, enable is %s' % enable)
        self.msg_buff.init()
    ...
    def set_sub_node(self):
        ...
        self.msg_buff.append('version', 'uint32', 50331648)
        duss_result, resp = self.event_client.send_sync(self.msg_buff)
        return (
         duss_result, resp)
    ...
    def add_gimbal_and_chassis_sub_msg(self, freq, uuid_list, callback):
        ...
        for index in range(len(uuid_list)):
            ...
            self.msg_buff.append('uuid_%d_h' % index, 'uint32', 131081)
        else:
            self.msg_buff.append('freq', 'uint16', freq)
            duss_result, resp = self.event_client.send_sync(self.msg_buff)
            if not duss_result != rm_define.DUSS_SUCCESS:
                if not resp['data'][0] != 0 or resp['data'][0] != 35:
...
class SDKModule(object):
    ...
    def sdk_on(self, mode):
        ...
        self.msg_buff.append('data', 'uint8', 1 | mode << 4 & 240)
        duss_result, resp = self.event_client.send_sync(self.msg_buff)
        return duss_result
...
class SpeechRecognizer(object):
    SPEECH_VOLUME_ENABLE = 1
    SPEECH_RECOGNIZER_START = 2
    SPPECH_TEXT_TO_SPEECH_START = 3
    SPPECH_TEXT_TO_SPEECH_STOP = 4
    ...
    def __init__(self, event_client):
        ...
        self.msg_buff.set_default_receiver(rm_define.hdvt_uav_id)
        self.msg_buff.set_default_moduleid(rm_define.system_id)
        self.msg_buff.set_default_cmdset(duml_cmdset.DUSS_MB_CMDSET_EDU_PLATFORM)
        self.task_id = random.randint(duml_cmdset.TASK_ID_MIN, duml_cmdset.TASK_ID_MAX)

    def speech_check_ctrl(self, ctrl):
        self.msg_buff.init()
        self.msg_buff.append('cmdid', 'uint8', self.SPEECH_VOLUME_ENABLE)
        self.msg_buff.append('ctrl', 'uint8', ctrl)
        self.msg_buff.cmd_id = duml_cmdset.DUSS_MB_CMD_EDUPLATFORM_SPEECH_DATA_SET
        self.msg_buff.receiver = rm_define.hdvt_uav_id
        duss_result, resp = self.event_client.send_sync(self.msg_buff)
        return duss_result

    def sub_speech_data_push(self, callback):
        cmd_set_id = duml_cmdset.DUSS_MB_CMDSET_EDU_PLATFORM << 8 | duml_cmdset.DUSS_MB_CMD_EDUPLATFORM_SPEECH_DATA_PUSH
        self.event_client.async_req_register(cmd_set_id, callback)

    def speech_recognizer_start_with_time(self, lan_type, wait_time):
        self.msg_buff.init()
        self.msg_buff.append('cmdid', 'uint8', self.SPEECH_RECOGNIZER_START)
        self.msg_buff.append('lan_type', 'uint8', lan_type)
        self.msg_buff.append('time', 'uint8', wait_time)
        self.msg_buff.cmd_id = duml_cmdset.DUSS_MB_CMD_EDUPLATFORM_SPEECH_DATA_SET
        self.msg_buff.receiver = rm_define.hdvt_uav_id
        duss_result, resp = self.event_client.send_sync(self.msg_buff, 30)
        return (
         duss_result, resp)

    def speech_text_to_speech_start(self, role_name, volume, speech_rate, pitch_rate, speech_text):
        self.msg_buff.init()
        self.msg_buff.append('cmdid', 'uint8', self.SPPECH_TEXT_TO_SPEECH_START)
        name_encoded_string = role_name.encode('utf-8')
        name_data = bytearray(name_encoded_string)
        self.msg_buff.append('role_name_len', 'uint8', len(name_data))
        for index in range(len(name_data)):
            self.msg_buff.append('role_name' + str(index), 'uint8', name_data[index])
        else:
            self.msg_buff.append('volume', 'uint8', volume)
            self.msg_buff.append('speech_rate', 'int16', speech_rate)
            self.msg_buff.append('pitch_rate', 'int16', pitch_rate)
            encoded_string = speech_text.encode('utf-8')
            speech_data = bytearray(encoded_string)
            self.msg_buff.append('text_len', 'uint16', len(speech_data))
            for index in range(len(speech_data)):
                self.msg_buff.append('text_data' + str(index), 'uint8', speech_data[index])
            else:
                self.msg_buff.cmd_id = duml_cmdset.DUSS_MB_CMD_EDUPLATFORM_SPEECH_DATA_SET
                self.msg_buff.receiver = rm_define.hdvt_uav_id
                duss_result, resp = self.event_client.send_sync(self.msg_buff, 30)
    ...
    def play_text_to_speech_task(self, role_name, volume, speech_rate, pitch_rate, speech_text):
        self.task_id = (self.task_id + 1) % duml_cmdset.TASK_ID_MAX
        self.msg_buff.init()
        self.msg_buff.append('task_id', 'uint8', self.task_id)
        self.msg_buff.append('task_ctrl', 'uint8', duml_cmdset.TASK_FREQ_10Hz)
        name_encoded_string = role_name.encode('utf-8')
        name_data = bytearray(name_encoded_string)
        self.msg_buff.append('role_name_len', 'uint8', len(name_data))
        for index in range(len(name_data)):
            ...
            for index in range(len(speech_data)):
                ...
                self.msg_buff.cmd_id = duml_cmdset.DUSS_MB_CMD_EDUPLATFORM_TEXT_TO_SPEECH_TASK
                self.msg_buff.receiver = rm_define.hdvt_uav_id
                event_task = {}
                event_task['task_id'] = self.task_id
                event_task['receiver'] = self.msg_buff.receiver
                event_task['cmd_set'] = duml_cmdset.DUSS_MB_CMDSET_EDU_PLATFORM
                event_task['cmd_id'] = duml_cmdset.DUSS_MB_CMD_EDUPLATFORM_TEXT_TO_SPEECH_TASK_PUSH
                duss_result, identify, resp = self.event_client.send_task_async_ret_resp(self.msg_buff, event_task, 30)
                if duss_result != rm_define.DUSS_SUCCESS:
                    pass
    ...
    def stop_text_to_speech(self):
        self.msg_buff.init()
        self.msg_buff.append('cmdid', 'uint8', self.SPPECH_TEXT_TO_SPEECH_STOP)
        self.msg_buff.cmd_id = duml_cmdset.DUSS_MB_CMD_EDUPLATFORM_SPEECH_DATA_SET
        self.msg_buff.receiver = rm_define.hdvt_uav_id
        duss_result, resp = self.event_client.send_sync(self.msg_buff)
        return duss_result
...
class IrDistanceSensor(object):
    TOF_MESAURE_CTRL_ID = 90
    TOF_DATA_PUSH_SUB_ID = 93
    TOF_DATA_GET_ID = 97
    TOF_DATA_PUSH_INFO_ID = 20
    ...
    def __init__(self, event_client):
        ...
        self.id_to_host_id = {0:rm_define.tof_id, 
         1:rm_define.tof1_id, 
         2:rm_define.tof2_id, 
         3:rm_define.tof3_id, 
         4:rm_define.tof4_id}
    ...
    def measure_ctrl(self, id, ctrl):
        ...
        self.msg_buff.append('tofid', 'uint8', 65)
        self.msg_buff.append('ctrl', 'uint8', ctrl)
...
class Servo(object):
    ...
    def __init__(self, event_client):
        ...
        self.msg_buff.set_default_cmdset(duml_cmdset.DUSS_MB_CMDSET_ROBOTIC_ARM)
        self.task_id = random.randint(duml_cmdset.TASK_ID_MIN, duml_cmdset.TASK_ID_MAX)
        self.id_to_host_id = {0:rm_define.servo_id, 
...
class RoboticArm(object):
    ...
    def __init__(self, event_client):
        ...
        self.msg_buff.set_default_cmdset(duml_cmdset.DUSS_MB_CMDSET_ROBOTIC_ARM)
        self.task_id = random.randint(duml_cmdset.TASK_ID_MIN, duml_cmdset.TASK_ID_MAX)
        self.id_to_host_id = {1: rm_define.robotic_arm_id}
...
class Serial(object):
    ...
    def set_serial_param(self, baud_rate, data_bit, odd_even, stop_bit, rx_enable, tx_enable, rx_buffer, tx_buffer):
        ...
        uart_config = (stop_bit_value & 1) << 7 | (odd_even_value & 3) << 5 | (data_bit_value & 3) << 3 | baud_rate_value & 7
        uart_enable = (tx_enable & 1) << 1 | rx_enable & 1
    ...
    def send_msg(self, len, buffer):
        self.msg_buff.init()
        self.msg_buff.append('msg_type', 'uint8', 2)
...
class TrackLine(object):
    ...
    def set_enable(self, ctrl):
        self.msg_buff.init()
        self.msg_buff.append('ctrl', 'uint8', ctrl)
        self.msg_buff.cmd_id = duml_cmdset.DUSS_MB_CMD_EDUPLATFORM_TRACKLINE_SET_ENABLE
        duss_result, resp = self.event_client.send_sync(self.msg_buff)
        return duss_result