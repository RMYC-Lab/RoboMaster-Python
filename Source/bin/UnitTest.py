...
if rm_path.get_scratch_log_enable() == 0:
    LOG_FILE_OUT_LEVEL = None
    LOG_STREAM_OUT_LEVEL = None

logger = rm_log.logger_init(logger, event_dji_system, LOG_FILE_OUT_LEVEL, LOG_STREAM_OUT_LEVEL)

def on_tools_log(msg):
    logger.info(msg)

tools.set_tools_msg_callback(on_tools_log)

logger.info('Current Working Dir: ' + os.getcwd())
for p in sys.path:
    logger.info('Seatch Path:' + p)


logger.info('lib Path:' + sys.prefix)

logger.info('exe Path:' + sys.executable)
...
logger.info('DJI SCRATCH CREATE pingMsg...')
...
logger.info('DJI SCRATCH CREATE SDKManager...')
...
#event_dji_system.start_monitor()

while not G_SCRIPT_FINISH:
    try:
        time.sleep(1)
        if event_dji_system.finished():
            break
    except Exception as e:
